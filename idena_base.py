...
def write_to_file(ip_addresses,i):
    b=i+1
    global json_str
...
def idena_stats(i):
    ...
    def idena_age(address):
        import requests
        import json
        url = 'https://api.idena.io/api/Identity/' + '' + address +'/Age'
        headers = {'Content-Type': 'application/json'}
        response = requests.post(url, headers=headers)
        a = json.loads(response.text)
        b = a['result']
    ...
    sheet.cell(row=int(i), column=6, value=idena_age(cell.value))
    sheet.cell(row=int(i), column=7, value=idena_identity(cell.value)['state'])
